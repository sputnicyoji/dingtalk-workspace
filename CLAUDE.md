...
- 不是任何 host 的专用集成方案（早期方向，已废弃，相关代码归档在 `legacy/`）
...
- 任何形式的 host 假设（代码路径里出现 host 名字就是 red flag）
...
4. **不得引入 host 假设**——任何代码路径里出现具体 host 名字都是 red flag
...
- "支持飞书/Slack" → 项目范围外
- "用 Rust 重写" → 已在架构阶段拒绝
- "主包加点某个 host 专用优化" → 破坏 host-agnostic 原则
